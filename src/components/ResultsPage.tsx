...
const getRowEntryClass = (entryColor: string, boldText: boolean) => {
  return `px-6 py-4 whitespace-nowrap text-s ${
    boldText ? "font-medium" : ""
  } text-gray-900 border-r border-gray-200 ${entryColor}`;
};
...
const transformSmiles = (smiles: string) => {
  ...
  return smiles.replace(
    /\[([nocspe])H\]/gi,
    (match, atom: keyof typeof atomMap) => {
...
const getRowEntryColor = (pscore: number) => {
  if (pscore === null || pscore === undefined || pscore < 0)
    return "bg-gray-300";
...
const displayPopupWindow = async (
  popupName: string,
...
const displayDrugDetails = async (
  scaffoldID: number,
  ...
  displayPopupWindow(
    popupName,
    popupWindowName,
    <DrugDetails scaffoldID={scaffoldID} scaffoldImage={scaffoldImage} />
  );
};
    ...
    <tr key={`${index}-${scaffoldIndex}`} className={rowColor}>
      ...
      {scaffoldImage ? (
        ...
        <td id="table-results" className={otherColClass}>
          Has no scaffolds <br></br>or &gt;maxRings
        </td>
      )}
      <td className={otherColClass}>{inDBString}</td>
